...
try {
  ...
  Bun.serve({
    ...
    async fetch(req) {
      try {
        ...
        const corsOrigin =
          normalizedOrigin && allowedUrls.includes(normalizedOrigin)
            ? origin!
            : allowedUrls[0];
        ...
        if (url.pathname === "/api/auth/google" && req.method === "POST") {
          const response = await authController.handleGoogle(req);
          Object.entries(corsHeaders).forEach(([k, v]) =>
            response.headers.set(k, v),
          );
          return response;
        ...
        if (url.pathname === "/api/auth/github" && req.method === "POST") {
          const response = await authController.handleGitHub(req);
          Object.entries(corsHeaders).forEach(([k, v]) =>
            response.headers.set(k, v),
          );
          return response;
  ...
  log(`[CRITICAL ERROR] Server failed to start: ${errorMsg}`);
}
