import {
  ...
  ISynthesizeOptions,
  ITtsResponse,
} from "../../core/repositories/ITtsService";
import { Voice } from "../../core/entities/Voice";
...
export class EdgeTtsService implements ITtsService {
  ...
  async synthesize(options: ISynthesizeOptions): Promise<ITtsResponse> {
    ...
    console.log(`[EdgeTtsService] Synthesizing with voice: ${voiceId}`);
    ...
    try {
      ...
      console.log(`[EdgeTtsService] Synthesis successful. Reading output...`);

      // Read the generated audio file
      const audio = await readFile(audioPath);
      console.log(
        `[EdgeTtsService] Audio file read. Size: ${audio.length} bytes`,
      );
      let marks: any[] = [];
